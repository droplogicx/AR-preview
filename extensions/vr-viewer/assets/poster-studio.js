...
(function () {
  ...
  var enabled = true;
  var settingsData = null;   // response from /api/settings — which image(s) we're allowed to cover
  ...
  function selectedVariantId() {
    ...
    var input =
      ...
      document.querySelector('.variant-input-wrap input[name="id"]') ||
      document.querySelector('.variant-wrapper input[name="id"]') ||
      document.querySelector('[data-variant-input][name="id"]');
    if (input && input.value) return String(input.value);

    // Try to find variant from radio/checkbox selections
    var checked = document.querySelector('.variant-input-wrap input:checked, .variant-wrapper input:checked, [data-variant-input]:checked');
   ...
   * markup, for example) keep more than one slide in the DOM at once. Where a
   * theme marks which one is actually the front slide, trust that over pure
   * size/position guessing. */
  function slidePreferenceScore(img) {
    var cell = img.closest && img.closest(
      '.product-main-slide, .slideshow-slide, .slider__slide, .slideshow__slide, [data-media-id]');
    if (!cell) return 0;
    var cls = cell.className || '';
    if (/\bstarting-slide\b/.test(cls)) return 2;
    if (/\bsecondary-slide\b|\binactive\b|\bis-hidden\b/.test(cls)) return -1;
    return 0;
  }
    ...
    for (var i = 0; i < HOST_SELECTORS.length; i++) {
      ...
      for (var h = 0; h < hosts.length; h++) {
        ...
        for (var j = 0; j < imgs.length; j++) {
          ...
          var score = slidePreferenceScore(img);
          var r = img.getBoundingClientRect();
          var area = r.width * r.height;
          /* prefer the theme's marked "front" slide; then the largest;
             break remaining ties by whichever sits highest */
          var better = score > bestScore ||
            (score === bestScore && (area > bestArea + 2000 ||
              (Math.abs(area - bestArea) <= 2000 && r.top < bestTop - 8)));
          if (better) { best = img; bestScore = score; bestArea = area; bestTop = r.top; }
        }
      }
  ...
  function buildOverlay() {
    ...
    overlay.innerHTML =
      ...
      '<button type="button" class="ps-reset" aria-label="Reset view" title="Reset view">\u21ba</button>' +
      '</div>' +
      '<div class="ps-hint">Drag to rotate</div>' +
  ...
  function bindEvents() {
    ...
    ['variant:update', 'variant:change', 'product:variant-change', 'variant:selected',
      ...
      'on:variant:change', 'on:variant:update', 'variantChange', 'variantUpdate',
      'product-variant-updated', 'variant_updated', 'swatch:changed'
    ...
    document.addEventListener('change', function (e) {
      ...
      if (t.closest('variant-picker, variant-selects, product-form') ||
        ...
        // Impulse (and similar themes): radios use the option's own name
        // ("Size", "Frame", "Border Options") and reference their form via
        // a `form="..."` attribute rather than being nested inside a
        // <form>/<product-form>, so none of the checks above catch them.
        // Every one of them carries data-variant-input, which is the
    ...
    window.addEventListener('pushstate', scheduleReconcile);
    window.addEventListener('replacestate', scheduleReconcile);
  ...
  function start() {
    ...
    var tries = 0;
    var lastVariantId = selectedVariantId();
    pollTimer = setInterval(function () {
      reconcile();
      tries++;
      // Check if variant ID changed in URL (some themes update URL silently)
      var currentVariantId = selectedVariantId();
      ...
      if (viewer && tries > 8) {
        clearInterval(pollTimer); pollTimer = setInterval(function () {
          var newVariantId = selectedVariantId();
      ...
      // if no image has ever been found yet, keep trying but at a slower,
      // low-cost pace rather than giving up outright — some themes (fade-in
      // animation libraries, deferred slideshows) take a while to settle
      else if (!primaryImage && tries > 80) {
        clearInterval(pollTimer); pollTimer = setInterval(function () {
          var newVariantId = selectedVariantId();
          if (newVariantId !== lastVariantId) {
            lastVariantId = newVariantId;
